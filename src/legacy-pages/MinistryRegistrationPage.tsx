...
const MinistryRegistrationPage = () => {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      if (userData.accountStatus !== 'verified') {
        toast.error(`Your ${deploymentLabels.ministryAdminShort.toLowerCase()} account must be approved by ${deploymentLabels.topAdminApprovalLower} first`);
        navigate('/dashboard');
        return;
      }
  ...
  const onSubmit = async (data: MinistryFormData) => {
    ...
    try {
      ...
      toast.info(
        `Your ministry is pending verification by the ${deploymentLabels.topAdminApprovalLower}. You will be notified once approved.`,
        { autoClose: 8000 }
      );
    ...
    <Container maxWidth="sm">
      ...
      >
        ...
        <Typography variant="body2" color="text.secondary" paragraph sx={{ textAlign: 'center' }}>
          ...
          After {deploymentLabels.topAdminApprovalLower} approval, you can approve staff to join your ministry.
        </Typography>
        ...
        <Paper elevation={3} sx={{ p: 4, mt: 3, width: '100%' }}>
          ...
          <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <TextField
              ...
              placeholder={isStateDeployment ? 'e.g., Rivers State Ministry of Finance' : 'e.g., Federal Ministry of Finance'}
              autoFocus
              {...register('name')}
              error={!!errors.name}
