...
const ministryInfoSchema = yup.object().shape({
  ...
  ministryLocation: isStateDeployment
    ? yup.string().optional()
    : yup.string().required('Headquarters location is required'),
});

// Combined schema for final submission
const fullSchema = personalInfoSchema.concat(ministryInfoSchema);

const steps = ['Personal Information', deploymentLabels.ministryInformationTitle];
...
const RegisterMinistryAdminPage = () => {
  ...
  const {
    ...
    mode: 'onChange',
    defaultValues: {
      ministryLocation: isStateDeployment ? deploymentState || '' : '',
    },
  });

  useEffect(() => {
    if (isStateDeployment && deploymentState) {
      setValue('ministryLocation', deploymentState);
    }
  }, [setValue]);
  ...
  const onSubmit = async (data: any) => {
    const resolvedLocation =
      isStateDeployment ? deploymentState?.trim() || String(data.ministryLocation || '').trim() : String(data.ministryLocation || '').trim();

    if (isStateDeployment && !resolvedLocation) {
      setError('This deployment must have a jurisdiction state configured.');
      toast.error('State jurisdiction is not configured for this portal.');
      return;
    }
    ...
    const registrationData: MinistryAdminRegistrationData = {
      ...
      ministryLocation: resolvedLocation,
    };
    ...
    try {
      ...
      toast.info(
        `Please verify your email. Your account and ministry will be reviewed by the ${deploymentLabels.topAdminTitle}.`,
        { autoClose: 10000 }
      );
                ...
                <Box>
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12}>
                      <TextField
                        ...
                        fullWidth
                        id="ministryName"
                        label={deploymentLabels.ministryEntityNameLabel}
                        placeholder={isStateDeployment ? 'e.g., Rivers State Ministry of Works' : 'e.g., Federal Ministry of Works'}
                    ...
                    <Grid item xs={12}>
                      <TextField
                        ...
                        label={deploymentLabels.officialMinistryEmailLabel}
                        type="email"
                        placeholder="e.g., info@works.gov.ng"
                        {...register('ministryOfficialEmail')}
                        error={!!errors.ministryOfficialEmail}
                        helperText={errors.ministryOfficialEmail?.message || `Official contact email for the ${deploymentLabels.ministryEntityLabel.toLowerCase()}`}
                        sx={inputStyles}
                      />
                    </Grid>

                    <Grid item xs={12} sm={isStateDeployment ? 12 : 6}>
                      <TextField
                        required
                        fullWidth
                        select
                        id="ministryType"
                        label={deploymentLabels.ministryEntityTypeLabel}
                        defaultValue=""
                      ...
                      <Grid item xs={12}>
                        ...
                        >
                          <Typography variant="body2">
                            <strong>Headquarters jurisdiction:</strong>{' '}
                            {deploymentLabels.jurisdiction}. Location is saved as{' '}
                            <strong>{deploymentState || '(not detected — contact support)'}</strong>.
                          </Typography>
                        </Alert>
                      </Grid>
                    ) : (
                  ...
                  >
                    ...
                    <Box component="ol" sx={{ pl: 2, mb: 0, '& li': { mb: 0.5, fontSize: '0.85rem' } }}>
                      ...
                      <li>{deploymentLabels.topAdminTitle} reviews your application</li>
