...
/** In state deployments, reports should group by ministry instead of the single deployment state. */
const groupByMinistry = (assets: any[]): { state: string; assets: any[] }[] => {
  const map = new Map<string, any[]>();
  assets.forEach((a) => {
    const key = a.ministry || a.ministryName || a.agencyName || 'Unspecified Ministry';
    if (!map.has(key)) map.set(key, []);
    map.get(key)!.push(a);
  });
  return Array.from(map.entries())
...
/** Ministry-scoped state reports should show agencies under that ministry. */
const groupByAgency = (assets: any[]): { state: string; assets: any[] }[] => {
  const map = new Map<string, any[]>();
  assets.forEach((a) => {
    const key = a.agency || a.staffAgencyName || a.agencyName || 'Unspecified Agency';
    if (!map.has(key)) map.set(key, []);
    map.get(key)!.push(a);
  });
  return Array.from(map.entries())
...
type ReportGroupingMode = 'state' | 'ministry' | 'agency';
type ReportGroup = { state: string; assets: any[] };

const groupAssetsForReport = (assets: any[], mode: ReportGroupingMode) => {
  if (mode === 'agency') return groupByAgency(assets);
  if (mode === 'ministry') return groupByMinistry(assets);
  return groupByState(assets);
};
...
const StateGroup: React.FC<{
  ...
  onExportPDF?: () => void; onExportExcel?: () => void; onPrint?: () => void;
}> = ({ state, assets, serial, highlighted = false, onClearHighlight, onExportPDF, onExportExcel, onPrint }) => {
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Tooltip title="Export this report section as PDF">
              <IconButton
                size="small"
                onClick={(e) => { e.stopPropagation(); onExportPDF?.(); }}
                sx={{ color: '#ef5350', border: '1px solid rgba(239,83,80,0.35)', p: 0.45, '&:hover': { backgroundColor: 'rgba(239,83,80,0.12)' } }}
              >
                <PictureAsPdf sx={{ fontSize: 15 }} />
              </IconButton>
            </Tooltip>
            <Tooltip title="Download this report section as Excel">
              <IconButton
                size="small"
                onClick={(e) => { e.stopPropagation(); onExportExcel?.(); }}
                sx={{ color: '#4caf50', border: '1px solid rgba(76,175,80,0.35)', p: 0.45, '&:hover': { backgroundColor: 'rgba(76,175,80,0.12)' } }}
              >
                <Download sx={{ fontSize: 15 }} />
              </IconButton>
            </Tooltip>
            <Tooltip title="Print this report section">
              <IconButton
                size="small"
...
const ReportsPage: React.FC = () => {
  ...
  const reportGroupingMode: ReportGroupingMode = isStateDeployment
    ? (isScopedToMinistry ? 'agency' : 'ministry')
    : 'state';
  const reportGroupLabel =
    reportGroupingMode === 'agency' ? 'Agency' :
    reportGroupingMode === 'ministry' ? 'Ministry' :
    'State';
  const reportGroupLabelPlural =
    reportGroupingMode === 'agency' ? 'Agencies' :
    reportGroupingMode === 'ministry' ? 'Ministries' :
    'States';
    ...
    (async () => {
      try {
        ...
        const [m, l] = await Promise.all([
          ...
          getUniqueLocations(
            isScopedToMinistry ? userData?.ministryId : undefined,
            isApprover ? userData?.state : undefined
          ),
        ]);
  ...
  const handleExportPDF = () => {
    ...
    doc.text(deploymentLabels.jurisdiction.toUpperCase(), pw / 2, 11, { align: 'center' });
    doc.setFontSize(11);
    ...
    if (generatedReport.type === 'asset_inventory') {
      ...
      autoTable(doc, {
        startY: 24,
        head: [[reportGroupLabel, 'Asset Count', 'Total Purchase Cost (₦)', 'Total Market Value (₦)']],
        body: groupAssetsForReport(data.assets || [], reportGroupingMode).map((g) => [
          g.state,
    ...
    for (let i = 1; i <= pages; i++) {
      ...
      doc.text(`${deploymentLabels.systemName} CONFIDENTIAL  |  Page ${i} of ${pages}  |  ${new Date().toLocaleDateString()}`,
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
      [deploymentLabels.jurisdiction.toUpperCase()], [deploymentLabels.systemName.toUpperCase()], [''],
    ...
    if (generatedReport.type === 'asset_inventory') {
      ...
      // Grand Summary sheet
      const summaryHeaders = [reportGroupLabel, 'Asset Count', 'Total Purchase Cost (₦)', 'Total Market Value (₦)'];
  ...
  const getGroupFileName = (group: ReportGroup, ext: 'pdf' | 'xlsx') => {
    const safeGroupName = group.state.replace(/[^a-z0-9]+/gi, '_').replace(/^_+|_+$/g, '');
    const safeReportTitle = (generatedReport?.title || 'Report').replace(/[^a-z0-9]+/gi, '_').replace(/^_+|_+$/g, '');
    return `GAMS_${safeReportTitle}_${safeGroupName}_${Date.now()}.${ext}`;
  };
  ...
  const handleExportGroupPDF = (group: ReportGroup) => {
    ...
    const doc = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a3' });
    const pw = doc.internal.pageSize.getWidth();
    const purchaseTotal = group.assets.reduce((s, a) => s + (Number(a.purchaseCost) || 0), 0);
    const marketTotal = group.assets.reduce((s, a) => s + (Number(a.marketValue) || 0), 0);

    doc.setFillColor(0, 80, 40); doc.rect(0, 0, pw, 38, 'F');
    doc.setTextColor(255, 255, 255); doc.setFontSize(16); doc.setFont('helvetica', 'bold');
    doc.text(deploymentLabels.jurisdiction.toUpperCase(), pw / 2, 11, { align: 'center' });
    doc.setFontSize(10);
    doc.text(`${generatedReport.title.toUpperCase()} - ${group.state.toUpperCase()}`, pw / 2, 22, { align: 'center' });
    doc.setFontSize(7.5); doc.setFont('helvetica', 'normal');
    doc.text(`Generated: ${generatedReport.generatedAt.toLocaleString()} | By: ${generatedReport.generatedBy}`, pw / 2, 32, { align: 'center' });
    doc.setTextColor(0, 0, 0);

    autoTable(doc, {
      startY: 44,
      head: [[reportGroupLabel, 'Assets', 'Total Purchase Cost', 'Total Market Value']],
      body: [[group.state, String(group.assets.length), formatCurrency(purchaseTotal), formatCurrency(marketTotal)]],
      theme: 'grid',
      headStyles: { fillColor: [0, 80, 40], textColor: [255, 255, 255], fontStyle: 'bold' },
    });

    autoTable(doc, {
      startY: (doc as any).lastAutoTable.finalY + 8,
      head: [groupExportHeaders],
      body: getGroupExportRows(group.assets),
      theme: 'grid',
      headStyles: { fillColor: [0, 60, 30], textColor: [255, 255, 255], fontSize: 6.2, fontStyle: 'bold', cellPadding: 1.6 },
      bodyStyles: { fontSize: 6.2, cellPadding: 1.4 },
      alternateRowStyles: { fillColor: [242, 250, 245] },
      styles: { overflow: 'linebreak', minCellWidth: 11 },
    });

    const pages = (doc as any).getNumberOfPages();
    for (let i = 1; i <= pages; i++) {
      doc.setPage(i); doc.setFontSize(6.5); doc.setTextColor(150, 150, 150);
      doc.text(`${deploymentLabels.systemName} | ${group.state} | Page ${i} of ${pages}`,
        doc.internal.pageSize.getWidth() / 2, doc.internal.pageSize.getHeight() - 5, { align: 'center' });
    }

    doc.save(getGroupFileName(group, 'pdf'));
  };

  const handleExportGroupExcel = (group: ReportGroup) => {
    if (!generatedReport) return;
    const wb = XLSX.utils.book_new();
    const purchaseTotal = group.assets.reduce((s, a) => s + (Number(a.purchaseCost) || 0), 0);
    const marketTotal = group.assets.reduce((s, a) => s + (Number(a.marketValue) || 0), 0);
    const summary = [
      ...
      [reportGroupLabel, group.state],
      ['Generated At', generatedReport.generatedAt.toLocaleString()],
      ['Generated By', generatedReport.generatedBy],
      ['Asset Count', group.assets.length],
      ['Total Purchase Cost', purchaseTotal],
      ['Total Market Value', marketTotal],
    ];
  ...
  const handlePrintGroup = (group: ReportGroup) => {
    if (!generatedReport) return;
    const purchaseTotal = group.assets.reduce((s, a) => s + (Number(a.purchaseCost) || 0), 0);
    const marketTotal = group.assets.reduce((s, a) => s + (Number(a.marketValue) || 0), 0);
    const rows = getGroupExportRows(group.assets)
      .map((row) => `<tr>${row.map((cell) => `<td>${String(cell || '').replace(/[&<>"']/g, (ch) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[ch] as string))}</td>`).join('')}</tr>`)
      .join('');
    ...
    printable.document.write(`
      <html>
        <head>
          <title>${group.state} ${generatedReport.title}</title>
          <style>
            body { font-family: Arial, sans-serif; color: #111; margin: 24px; }
            h1 { font-size: 18px; margin: 0 0 4px; }
            h2 { font-size: 14px; margin: 0 0 16px; color: #006038; }
            .meta { font-size: 11px; margin-bottom: 16px; color: #555; }
            .totals { display: flex; gap: 24px; margin-bottom: 16px; font-size: 12px; }
            table { width: 100%; border-collapse: collapse; font-size: 10px; }
            th { background: #006038; color: #fff; text-align: left; }
        ...
        <body>
          <h1>${deploymentLabels.jurisdiction}</h1>
          <h2>${generatedReport.title} - ${group.state}</h2>
          <div class="meta">Generated: ${generatedReport.generatedAt.toLocaleString()} | By: ${generatedReport.generatedBy}</div>
          <div class="totals">
            <strong>Assets: ${group.assets.length}</strong>
            <strong>Purchase Cost: ${formatCurrency(purchaseTotal)}</strong>
            <strong>Market Value: ${formatCurrency(marketTotal)}</strong>
          </div>
          <table>
            <thead><tr>${groupExportHeaders.map((h) => `<th>${h}</th>`).join('')}</tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </body>
      </html>
        ...
        <Grid item xs={12} md={isAdmin ? 4 : 6}>
          <FormControl fullWidth size="small">
            ...
            <Select value={singleStatus} onChange={(e) => setSingleStatus(e.target.value as string)} input={<OutlinedInput label="Status" />}>
              ...
              <MenuItem value="submitted_to_federal">{deploymentLabels.sentToTopAdmin}</MenuItem>
        ...
        <Box>
          <Grid container spacing={3} sx={{ mb: 4 }}>
            {[
              ...
              { label: `${reportGroupLabelPlural} Covered`, value: groups.length.toString(),                 grad: 'linear-gradient(135deg,#1565c0,#0d47a1)' },
          ...
          {/* Reporting group summary table */}
          <SectionHeading
            icon={isStateDeployment ? <AccountBalance /> : <LocationOn />}
            title={`Summary by ${reportGroupLabel}`}
            count={groups.length}
          />
          <TableContainer component={Paper} sx={{ mb: 4, border: '1px solid rgba(0,135,81,0.2)' }}>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TH minWidth={220}>{reportGroupLabel}</TH>
              ...
              <TableBody>
                {groups.map((group, idx) => {
                  ...
                  // Build per-category breakdown for this reporting group
                      ...
                      <TableRow sx={{ backgroundColor: idx % 2 === 0 ? 'rgba(0,40,20,0.4)' : 'rgba(0,20,10,0.3)', '&:hover': { backgroundColor: 'rgba(0,135,81,0.1)' } }}>
                        ...
                        <TableCell sx={{ py: 0.5, px: 1.5, borderBottom: '1px solid rgba(255,255,255,0.05)', whiteSpace: 'nowrap' }}>
                          <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                            <Tooltip title={`Download ${group.state} report`}>
                              <IconButton
                                size="small"
                            ...
                            <Tooltip title={`Export ${group.state} report as PDF`}>
                              <IconButton
                                size="small"
                            ...
                            <Tooltip title={`Print ${group.state} report`}>
                              <IconButton
                                size="small"
                              ...
                              <Table size="small">
                                <TableHead>
                                  <TableRow sx={{ backgroundColor: 'rgba(0,60,30,0.6)' }}>
                                    {['Asset Category', 'Total Assets', 'Total Purchase Cost', 'Total Market Value', `% of ${reportGroupLabel}`].map((h) => (
                                      <TableCell key={h} sx={{
          ...
          {groups.map((group) => {
            ...
            return (
              <StateGroup
                ...
                onExportPDF={() => handleExportGroupPDF(group)}
                onExportExcel={() => handleExportGroupExcel(group)}
        ...
        {!loading && generatedReport && (
          <Paper sx={{ p: 3 }}>
            ...
            <Tabs value={activeTab} onChange={(_, v) => setActiveTab(v)} sx={{ mb: 2 }} variant="scrollable" scrollButtons="auto">
              ...
              <Tab icon={<TableChart />} label={`Asset Register by ${reportGroupLabel}`} iconPosition="start" />
