 ...
 * @param maxResults - Maximum number of assets to fetch (default 5000)
 * @returns Array of all assets in the system
 */
export const getAllAssets = async (maxResults: number = 5000): Promise<Asset[]> => {
  try {
    const assetsRef = collection(db, COLLECTIONS.ASSETS);
    const q = query(assetsRef, limit(maxResults));
    const querySnapshot = await getDocs(q);
...
export const getAssetsPaginated = async (
  ...
  try {
    ...
    if (lastDocument) {
      q = query(assetsRef, orderBy('uploadTimestamp', 'desc'), startAfter(lastDocument), limit(pageSize));
    } else {
      q = query(assetsRef, orderBy('uploadTimestamp', 'desc'), limit(pageSize));
    }
...
export const approveAsset = async (
  ...
  try {
    ...
    // NEW WORKFLOW: Approver approves and sends to Ministry Admin for review
    ...
    if (approverEmail && agencyName && asset) {
      await logAction({
        ...
        resourceType: 'asset',
        resourceId: assetId,
        details: `Approved asset and sent to Ministry Admin for review: ${asset.description} (${asset.assetId})`,
        metadata: {
...
export interface AdminDashboardStats {
  ...
  statusCounts: {
    approved: number;
    pending: number;
    pending_ministry_review: number;
    submitted_to_federal: number;
    rejected: number;
  };
...
export const getAdminDashboardStats = async (): Promise<AdminDashboardStats> => {
  try {
    ...
    // Federal admin should only see assets that have reached federal level
    const assets = allAssets.filter(a =>
      ['submitted_to_federal', 'approved', 'rejected'].includes(a.status)
    );
    ...
    const statusCounts = {
      approved: assets.filter(a => a.status === 'approved').length,
      pending: 0,
      pending_ministry_review: 0,
      submitted_to_federal: assets.filter(a => a.status === 'submitted_to_federal').length,
      rejected: assets.filter(a => a.status === 'rejected').length,
    };
...
/**
 * Ministry Admin sends an asset to Federal Admin for final review
 * @param assetId - Document ID of the asset
 * @param ministryAdminId - User ID of the ministry admin
 * @param ministryAdminEmail - Email for audit logging
 * @param agencyName - Agency name for audit logging
 */
export const submitAssetToFederal = async (
  assetId: string,
  ministryAdminId: string,
  ministryAdminEmail?: string,
  agencyName?: string
  ...
  try {
    ...
    if (asset.status !== 'pending_ministry_review') {
      throw new Error(
        `Cannot submit asset with status "${asset.status}". Asset must be in "pending_ministry_review" status.`
      );
    }

    await updateDoc(assetRef, {
      status: 'submitted_to_federal',
      submittedToFederalBy: ministryAdminId,
      submittedToFederalAt: Timestamp.now(),
    });
    ...
    if (ministryAdminEmail && agencyName) {
      await logAction({
        ...
        agencyName,
        userRole: 'ministry-admin',
        action: 'asset.submit_to_federal',
        resourceType: 'asset',
        resourceId: assetId,
        details: `Submitted asset to Federal Admin: ${asset.description} (${asset.assetId})`,
        metadata: {
          assetId: asset.assetId,
          category: asset.category,
          submittedBy: {
    ...
    console.error('Error submitting asset to federal:', error);

    if (error.message?.includes('Missing or insufficient permissions')) {
      throw new Error(
        'Permission denied: You do not have access to submit this asset.'
      );
    }
    ...
    throw new Error(
      error.message || 'Failed to submit asset to federal. Please try again.'
    );
  }
};

/**
 * Ministry Admin sends all approved assets to Federal Admin
 * @param ministryId - Ministry ID
 * @param ministryAdminId - User ID of the ministry admin
 * @param ministryAdminEmail - Email for audit logging
 * @param agencyName - Agency name for audit logging
 * @returns Number of successfully submitted assets
 */
export const submitAllAssetsToFederal = async (
  ministryId: string,
  ministryAdminId: string,
  ministryAdminEmail?: string,
  agencyName?: string
  ...
  try {
    const pendingAssets = await getAssetsForMinistryReview(ministryId);
    ...
    console.error('Error submitting all assets to federal:', error);
    throw new Error('Failed to submit assets to federal. Please try again.');
  }
};
