...
export const fetchAssetsForReport = async (
  ...
  try {
    ...
    // Query with ministry filter for ministry-admin (required by Firestore rules)
    const q = isMinistryAdmin && userMinistryId
    ...
    // Additional ministry-level filtering (for admin filtering by specific ministries)
    if (!isMinistryAdmin) {
...
export const fetchAuditLogsForReport = async (
  ...
  try {
    ...
    // Ministry admin can only read their own audit logs per Firestore rules
    const q = isMinistryAdmin && userId
      ? query(logsRef, where('userId', '==', userId), orderBy('timestamp', 'desc'))
...
export const generateAuditReport = async (
  ...
  const logs = await fetchAuditLogsForReport(filters, isMinistryAdmin, userMinistryId, userId);
...
export const generateReport = async (
  ...
  switch (filters.reportType) {
    ...
    case 'audit_compliance':
      data = await generateAuditReport(filters, isMinistryAdmin, userMinistryId, userId);
      break;
 ...
 * @param ministryId - If provided, only fetch locations from this ministry's assets
 */
