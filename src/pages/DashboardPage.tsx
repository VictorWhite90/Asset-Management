...
const DashboardPage: React.FC = () => {
  ...
  const fetchDashboardStats = async () => {
    ...
    try {
      ...
      if (userData.role === 'agency' && isAccountVerified) {
        ...
        // Federal admin should only see assets that have been submitted to them or already processed
  ...
  const submittedToFederalAssets = assets.filter((a) => a.status === 'submitted_to_federal').length;
  ...
  const uniqueMinistries = new Set(assets.map((a) => a.ministryId).filter(Boolean)).size;
  ...
  return (
    ...
    <AppLayout>
      <Container maxWidth="xl">
              ...
              <Grid container spacing={3} sx={{ mb: 4 }}>
                ...
                {userData?.role === 'admin' && (
                  <>
                    ...
                    <Grid item xs={12} sm={6} md={3}>
                      <Card sx={{ background: 'linear-gradient(135deg, #7b1fa2 0%, #6a1b9a 100%)', border: 'none' }}>
                        <CardContent>
                          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <Box>
                              <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.8)' }}>
                                ...
                                <Pie
                                  data={(() => {
                                    const statusConfig: Record<string, { label: string; color: string }> = {
                                      submitted_to_federal: { label: 'Awaiting Review', color: '#7b1fa2' },
        ...
        {/* Ministry Admin - Management Actions */}
        {userData?.role === 'ministry-admin' &&
         currentUser?.emailVerified &&
         userData?.isMinistryOwner && (
          ...
          >
            ...
            <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)', mb: 2, fontSize: { xs: '0.8rem', sm: '0.875rem' } }}>
              Manage staff, review assets, and generate reports for your ministry
            </Typography>
            <Grid container spacing={{ xs: 1.5, sm: 2 }}>
              <Grid item xs={6} sm={3}>
                <Button
                  ...
                  size="medium"
                  sx={{ fontSize: { xs: '0.7rem', sm: '0.8rem' }, py: { xs: 1, sm: 1.5 } }}
                  onClick={() => {
                    sessionStorage.setItem('ministryAdminTab', '1');
                    navigate('/ministry-admin/dashboard');
                  }}
                >
                  Manage Staff
                </Button>
              ...
              <Grid item xs={6} sm={3}>
                <Button
                  ...
                  onClick={() => {
                    sessionStorage.setItem('ministryAdminTab', '2');
                    navigate('/ministry-admin/dashboard');
                  }}
                >
                  View Assets
              ...
              <Grid item xs={6} sm={3}>
                <Button
                  ...
                  onClick={() => {
                    sessionStorage.setItem('ministryAdminTab', '2');
                    sessionStorage.setItem('ministryAdminAssetTab', '1');
                    navigate('/ministry-admin/dashboard');
                  }}
                >
                  Assets History
