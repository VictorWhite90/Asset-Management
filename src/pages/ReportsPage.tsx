...
const ReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadFilterOptions = async () => {
      try {
        ...
        const ministriesData = await getMinistriesForFilter();
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={isAdmin ? 4 : 6}>
          <FormControl fullWidth size="small">
            ...
            <Select
              value={filters.assetTypes?.[0] || ''}
              onChange={(e) => {
                setFilters({
                  ...filters,
                  assetTypes: e.target.value ? [e.target.value as string] : [],
                });
              }}
          ...
          <FormControl fullWidth size="small">
            ...
            <Select
              ...
              onChange={(e) => {
                setFilters({
                  ...filters,
                  statuses: e.target.value ? [e.target.value as string] : [],
                });
              }}
            ...
            >
              ...
              {ASSET_STATUSES.map((status: string) => {
                const labelMap: Record<string, string> = {
                  pending: 'Pending',
                  pending_ministry_review: 'Ministry Review',
                  submitted_to_federal: 'Submitted to Federal',
                  approved: 'Approved',
                  rejected: 'Rejected',
                };
                return (
  ...
  const renderDataTablesTab = () => {
    ...
    if (generatedReport.type === 'asset_inventory') {
      ...
      return (
        <Box>
          {/* By Type Table with expandable asset details */}
          <Typography variant="h6" sx={{ color: '#00ff88', mb: 2 }}>
            Assets by Type
          </Typography>
          <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.5)', mb: 2 }}>
            Click on a row to see individual asset details
          </Typography>
          <TableContainer component={Paper} sx={{ mb: 4 }}>
            <Table size="small">
              ...
              <TableBody>
                {data.byType.map((row) => {
                  const isExpanded = expandedTypes.has(row.name);
                  const typeAssets = data.assets.filter((a) => a.type === row.name);
                  return (
                      ...
                      <TableRow key={`${row.name}-details`}>
                        <TableCell colSpan={4} sx={{ py: 0, px: 0 }}>
                          <Collapse in={isExpanded} timeout="auto" unmountOnExit>
                            <Box sx={{ p: 2, backgroundColor: 'rgba(0, 135, 81, 0.05)' }}>
                              <Table size="small">
                                <TableHead>
                                  <TableRow>
                                    <TableCell sx={{ color: '#00ff88', fontWeight: 'bold' }}>Asset Name</TableCell>
                                    <TableCell sx={{ color: '#00ff88', fontWeight: 'bold' }}>Location</TableCell>
                                    <TableCell sx={{ color: '#00ff88', fontWeight: 'bold' }}>Status</TableCell>
                                    <TableCell sx={{ color: '#00ff88', fontWeight: 'bold' }}>Condition</TableCell>
                                    <TableCell sx={{ color: '#00ff88', fontWeight: 'bold' }} align="right">Purchase Cost</TableCell>
                                    <TableCell sx={{ color: '#00ff88', fontWeight: 'bold' }} align="right">Current Value</TableCell>
                                  </TableRow>
                                </TableHead>
                                <TableBody>
                                      ...
                                      <TableCell>
                                        <Chip
                                          ...
                                          sx={{
                                            ...
                                            backgroundColor:
                                              asset.status === 'approved' ? 'rgba(76, 175, 80, 0.15)' :
                                              asset.status === 'submitted_to_federal' ? 'rgba(33, 150, 243, 0.15)' :
                                              asset.status === 'pending_ministry_review' ? 'rgba(33, 150, 243, 0.15)' :
                                              asset.status === 'rejected' ? 'rgba(244, 67, 54, 0.15)' :
                                              'rgba(255, 152, 0, 0.15)',
                                            color:
                                              asset.status === 'approved' ? '#4caf50' :
                                              asset.status === 'submitted_to_federal' ? '#2196f3' :
                                              asset.status === 'pending_ministry_review' ? '#2196f3' :
                                              asset.status === 'rejected' ? '#f44336' :
                                              '#ff9800',
                                          }}
                                        />
                                      </TableCell>
                                      <TableCell>{asset.condition || 'N/A'}</TableCell>
                                      <TableCell align="right">{formatCurrency(asset.acquisitionCost)}</TableCell>
