...
const ViewUploadsPage: React.FC = () => {
  ...
  const getApprovedAssets = () => {
    return assets.filter(
      (a) => (a.status === 'approved' || a.status === 'pending_ministry_review' || a.status === 'submitted_to_federal') && matches(a, searchTerm)
    );
  };
  ...
  const getStatusChip = (asset: Asset) => {
    switch (asset.status) {
      ...
      case 'pending_ministry_review':
        return <Chip icon={<PendingActions />} label="Sent to Ministry" color="info" size="small" />;
      case 'submitted_to_federal':
        return <Chip icon={<PendingActions />} label="Sent to Federal" color="warning" size="small" />;
      case 'approved':
          ...
          <DialogContent sx={{ mt: 2 }}>
            {selectedAsset && (
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                {(selectedAsset.status === 'pending_ministry_review' || selectedAsset.status === 'submitted_to_federal') && selectedAsset.approvedAt && (
                  <Box>
                    <Typography variant="caption" sx={{ color: '#aaa' }}>
                      Reviewed by Approver on
                    </Typography>
                    <Typography variant="body2">
                ...
                {selectedAsset.status === 'submitted_to_federal' && selectedAsset.submittedToFederalAt && (
                  <Box>
                    <Typography variant="caption" sx={{ color: '#aaa' }}>
                      Sent to Federal Admin on
                    </Typography>
                    <Typography variant="body2">
                      {format(selectedAsset.submittedToFederalAt.toDate(), 'MMM dd, yyyy hh:mm a')}
                    </Typography>
                  </Box>
                )}
