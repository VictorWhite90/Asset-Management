...
import {
  ...
  submitAssetToFederal,
  submitAllAssetsToFederal,
...
const MinistryAdminDashboardPage = () => {
  ...
  // Read tab from sessionStorage on every navigation to this page
  useEffect(() => {
    const savedTab = sessionStorage.getItem('ministryAdminTab');
    if (savedTab) {
      setTabValue(parseInt(savedTab, 10));
      sessionStorage.removeItem('ministryAdminTab');
    }
    const savedAssetTab = sessionStorage.getItem('ministryAdminAssetTab');
    if (savedAssetTab) {
      setAssetTabValue(parseInt(savedAssetTab, 10));
      sessionStorage.removeItem('ministryAdminAssetTab');
    }
  }, [location.key]);
  ...
  const handleSubmitToFederal = () => {
    if (pendingMinistryReviewAssets.length === 0) {
      toast.warning('No assets ready to submit to federal');
      return;
    }
  ...
  const handleSubmitToFederalConfirm = async () => {
    ...
    setActionLoading(true);
    try {
      const result = await submitAllAssetsToFederal(
        userData.ownedMinistryId,
        currentUser.uid,
        currentUser.email || '',
        userData.agencyName
      );
      ...
      if (result.success > 0) {
        toast.success(`${result.success} asset(s) sent to Federal Admin`);
      }
      if (result.failed > 0) {
        toast.warning(`${result.failed} asset(s) failed to submit`);
      }

      setSubmitDialogOpen(false);
      await loadData();
    } catch (err: any) {
      console.error('Error submitting assets to federal:', err);
      toast.error(err.message || 'Failed to submit assets to federal');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  // Send individual asset to Federal Admin
  const handleSendAssetToFederal = async (asset: Asset) => {
    if (!currentUser || !userData || !userData.ownedMinistryId) {
      toast.error('User context missing');
      return;
    }
    ...
    setActionLoading(true);
    try {
      await submitAssetToFederal(
        asset.id || '',
        currentUser.uid,
        currentUser.email || '',
        userData.agencyName
      );
      toast.success(`Asset "${asset.assetId}" sent to Federal Admin`);
      await loadData();
    } catch (err: any) {
      console.error('Error sending asset to federal:', err);
      toast.error(err.message || 'Failed to send asset to federal');
    } finally {
      setActionLoading(false);
    }
  };
    ...
    <AppLayout>
      <Container maxWidth="lg">
          ...
          <TabPanel value={tabValue} index={0}>
                  ...
                  <CardContent>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <Box>
                        ...
                        <Typography variant="h3" sx={{ color: '#FFFFFF', fontWeight: 700 }}>
                          {pendingMinistryReviewAssets.length}
                        </Typography>
                      </Box>
            ...
            {allAssets.length > 0 && (
              <Grid container spacing={3} sx={{ mb: 4 }}>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  >
                    <Typography variant="h6" sx={{ color: '#00ff88', mb: 2, fontWeight: 600, fontSize: '1rem' }}>
                        ...
                        <Pie
                          data={(() => {
                            ...
                            const statusConfig: Record<string, { label: string; color: string }> = {
                              ...
                              pending_ministry_review: { label: 'Pending Review', color: '#2196f3' },
                              submitted_to_federal: { label: 'Sent to Federal', color: '#1976d2' },
                              approved: { label: 'Approved', color: '#4caf50' },
                              rejected: { label: 'Rejected', color: '#f44336' },
                            };
                            allAssets.forEach((a) => {
                              const cfg = statusConfig[a.status] || { label: a.status, color: '#aaa' };
                              if (!statusMap[a.status]) statusMap[a.status] = { name: cfg.label, count: 0, color: cfg.color };
                        ...
                        >
                          {(() => {
                            const statusConfig: Record<string, string> = {
                              pending: '#ff9800',
                              pending_ministry_review: '#2196f3',
                              submitted_to_federal: '#1976d2',
                              approved: '#4caf50',
                              rejected: '#f44336',
                            };
                            const statuses = [...new Set(allAssets.map((a) => a.status))];
                            return statuses.map((status, i) => (
                              <Cell key={i} fill={statusConfig[status] || '#aaa'} />
                            ));
                          })()}
                        </Pie>
                        <RechartsTooltip
                          contentStyle={{ backgroundColor: '#0d2818', border: '1px solid rgba(0,135,81,0.3)', color: '#fff' }}
                        />
                      </PieChart>
                    </ResponsiveContainer>
                  </Paper>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  >
                    ...
                    <ResponsiveContainer width="100%" height={280}>
                      ...
                      >
                        <XAxis type="number" stroke="rgba(255,255,255,0.5)" tick={{ fill: 'rgba(255,255,255,0.7)', fontSize: 12 }} allowDecimals={false} />
                        <YAxis type="category" dataKey="name" stroke="rgba(255,255,255,0.5)" tick={{ fill: 'rgba(255,255,255,0.7)', fontSize: 12 }} width={100} />
                        <RechartsTooltip
                          contentStyle={{ backgroundColor: '#0d2818', border: '1px solid rgba(0,135,81,0.3)', color: '#fff' }}
                        />
                        <Bar dataKey="count" fill="#008751" radius={[0, 4, 4, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </Paper>
                </Grid>
            ...
            <Grid container spacing={2} sx={{ mb: 3 }}>
              ...
              <Grid item xs={12} sm={6} md={4}>
                ...
                >
                  Send to Federal ({pendingMinistryReviewAssets.length})
                </Button>
              </Grid>
            </Grid>
          ...
          <TabPanel value={tabValue} index={2}>
            ...
            <Paper elevation={0} sx={{ backgroundColor: 'transparent', mb: 2 }}>
              ...
              >
                ...
                />
                <Tab
                  label={`Sent to Federal (${submittedToFederalAssets.length})`}
                  id="asset-tab-1"
                />
                <Tab
                ...
                <>
                  <Alert severity="info" sx={{ mb: 2 }}>
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>
                      {pendingMinistryReviewAssets.length} asset(s) ready to send to Federal Admin
                    </Typography>
                  </Alert>
                      ...
                      <TableBody>
                        {pendingMinistryReviewAssets.map((asset) => (
                          <TableRow key={asset.id} hover>
                            ...
                            <TableCell align="right">
                              <Tooltip title="Send to Federal Admin">
                                <IconButton
                                  size="small"
                                  onClick={() => handleSendAssetToFederal(asset)}
                                  disabled={actionLoading}
                                  sx={{ color: '#008751', '&:hover': { backgroundColor: 'rgba(0, 135, 81, 0.1)' } }}
                                >
                                  <Send />
                                </IconButton>
                              </Tooltip>
            ...
            <TabPanel value={assetTabValue} index={1}>
              <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography variant="body2" sx={{ color: 'rgba(255, 255, 255, 0.6)' }}>
                  Assets submitted to Federal Admin for review
                </Typography>
              </Box>

              {submittedToFederalAssets.length === 0 ? (
                <Alert severity="info">No assets have been sent to Federal Admin yet</Alert>
              ) : (
                <>
                  <Paper
                    elevation={0}
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} md={6}>
                        ...
                        <Typography variant="h4" sx={{ color: '#FFFFFF', fontWeight: 600 }}>
                          {formatCurrency(calculateTotalValue(submittedToFederalAssets))}
                        </Typography>
                      </Grid>
                    </Grid>
                  </Paper>
                ...
                <TableContainer component={Paper}>
                  <Table>
                    ...
                    <TableBody>
                      {allAssets.map((asset) => {
                        let statusColor = '#aaa';
                        let statusLabel: string = asset.status;

                        if (asset.status === 'pending_ministry_review') {
                          statusColor = '#2196f3';
                          statusLabel = 'Pending Review';
                        } else if (asset.status === 'submitted_to_federal') {
                          statusColor = '#1976d2';
                          statusLabel = 'Sent to Federal';
        ...
        >
          ...
          <DialogContent>
            <Typography variant="body2" sx={{ color: 'rgba(255, 255, 255, 0.7)', mb: 2 }}>
              Sending <strong>{pendingMinistryReviewAssets.length} asset{pendingMinistryReviewAssets.length > 1 ? 's' : ''}</strong> to Federal Administrator for review
            </Typography>
          ...
          <DialogActions>
            ...
            <Button onClick={handleSubmitToFederalConfirm} variant="contained" disabled={actionLoading} sx={{ backgroundColor: '#008751' }}>
              {actionLoading ? 'Sending...' : 'Send to Federal'}
            </Button>
          </DialogActions>
        </Dialog>
